...
public class CodeVerification extends AppCompatEditText {
    ...
    public CodeVerification(Context context) {
        super(context);
        init(context , null);
    }

    public CodeVerification(Context context, AttributeSet attrs) {
        super(context, attrs , R.attr.verificationCodeStyle);
        init(context , attrs);
    }

    public CodeVerification(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        TypedArray typedArray = theme.obtainStyledAttributes(attrs, R.styleable.CodeVerification, R.attr.verificationCodeStyle, 0);
        viewType = typedArray.getInt(R.styleable.CodeVerification_viewType, VIEW_TYPE_RECTANGLE);
        otpViewItemCount = typedArray.getInt(R.styleable.CodeVerification_itemCount, DEFAULT_COUNT);
        otpViewItemHeight = (int) typedArray.getDimension(R.styleable.CodeVerification_itemHeight, res.getDimensionPixelSize(R.dimen.verification_item_size));
        otpViewItemWidth = (int) typedArray.getDimension(R.styleable.CodeVerification_itemWidth, res.getDimensionPixelSize(R.dimen.verification_item_size));
        otpViewItemSpacing = typedArray.getDimensionPixelSize(R.styleable.CodeVerification_itemSpacing, res.getDimensionPixelSize(R.dimen.verification_item_spacing));
        otpViewItemRadius = (int) typedArray.getDimension(R.styleable.CodeVerification_itemRadius, 0);
        lineWidth = (int) typedArray.getDimension(R.styleable.CodeVerification_lineWidth, res.getDimensionPixelSize(R.dimen.verification_item_line_width));
        lineColor = typedArray.getColorStateList(R.styleable.CodeVerification_lineColor);
        isCursorVisible = typedArray.getBoolean(R.styleable.CodeVerification_android_cursorVisible, true);
        cursorColor = typedArray.getColor(R.styleable.CodeVerification_cursorColor, getCurrentTextColor());
        cursorWidth = typedArray.getDimensionPixelSize(R.styleable.CodeVerification_cursorWidth, res.getDimensionPixelSize(R.dimen.verification_cursor_width));
        itemBackground = typedArray.getDrawable(R.styleable.CodeVerification_android_itemBackground);
        hideLineWhenFilled = typedArray.getBoolean(R.styleable.CodeVerification_hideLineWhenFilled, false);
        rtlTextDirection = typedArray.getBoolean(R.styleable.CodeVerification_rtlTextDirection, false);
        typedArray.recycle();
        if(lineColor != null){
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(widthMode == MeasureSpec.EXACTLY){
            ...
            int boxesWidth = (otpViewItemCount - 1) * otpViewItemSpacing + otpViewItemCount * otpViewItemWidth;
        ...
        height = heightMode == MeasureSpec.EXACTLY ? heightSize : boxHeight + getPaddingTop() + getPaddingBottom();
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        if(text.length() == otpViewItemCount && onCompletionListener != null){
            onCompletionListener.onFinishEnterCode(text.toString());
        }
        makeBlink();
    ...
    private void drawOtpView(Canvas canvas) {
        ...
        if(isFocused() && getText() != null && getText().length() != otpViewItemCount && viewType == VIEW_TYPE_RECTANGLE){
    ...
    private int getLineColorForState(int... states) {
        return lineColor != null ? lineColor.getColorForState(states, cursorLineColor) : cursorLineColor;
    }
    ...
    private void drawOtpLine(Canvas canvas, int i) {
        ...
        itemLineRect.set(itemBorderRect.left - halfLineWidth, itemBorderRect.bottom - halfLineWidth, itemBorderRect.right + halfLineWidth, itemBorderRect.bottom + halfLineWidth);
    ...
    private void updateItemRectF(int i) {
        float halfLineWidth = ((float) lineWidth) / 2;
        float left = getScrollX() + ViewCompat.getPaddingStart(this) + i * (otpViewItemSpacing + otpViewItemWidth) + halfLineWidth;
    ...
    private void updateColors() {
        ...
        int color = lineColor != null ? lineColor.getColorForState(getDrawableState(), 0) : getCurrentTextColor();
    ...
    public void setCompletionListener(OnCompletionListener otpCompletionListener) {
        this.onCompletionListener = otpCompletionListener;
    }
    ...
    private void updateCursorHeight() {
        int delta = 2 * dpToPx();
        cursorHeight = otpViewItemHeight - getTextSize() > delta ? getTextSize() + delta : getTextSize();
    }
